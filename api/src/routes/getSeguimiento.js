...
router.get("/seguimiento/:userId", async (req, res) => {
  try {
    ...
    if (!userSeguimientos || userSeguimientos.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron seguimientos para este usuario" });
    }
    ...
    res.status(200).json({
      message: "Seguimientos encontrados exitosamente",
      seguimientos: formattedData,
    });
  } catch (error) {
...
router.get("/userCourse/:userId", async (req, res) => {
  try {
    ...
    if (!userInscripciones || userInscripciones.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron inscripciones para este usuario" });
    }
...
router.get("/usuariosPorCurso/:cursoId", async (req, res) => {
  try {
    ...
    if (!usuariosInscritos || usuariosInscritos.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron usuarios para este curso" });
    }

    // Extraer los userSub de los resultados
    const usersInCurso = usuariosInscritos.map(
      (inscripcion) => inscripcion.userSub
    );
...
router.post("/userCourse", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Usuario asignado al curso e inscrito con éxito",
        seguimiento,
        inscripcion,
      });
  } catch (error) {
